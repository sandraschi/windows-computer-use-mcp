...
const TOOLS = [
	{
		...
		ops: "list, find, focus, maximize, close, position",
		phase: "core",
	},
	{
		name: "automation_elements",
		ops: "click, set_text, list, info, get_text, verify",
		phase: "1",
	},
	{
		name: "automation_mouse",
		ops: "click, double_click, right_click, scroll, drag, hover",
		phase: "core",
	},
	{
		name: "automation_keyboard",
		ops: "type, hotkey, press, hold",
		phase: "core",
	},
	{
		name: "automation_visual",
		ops: "screenshot, extract_text (OCR), find_image, highlight",
		phase: "core",
	},
	{
		name: "automation_assert",
		ops: "hash, diff, wait_stable, assert_changed, assert_text",
		phase: "core",
	},
	{
		name: "automation_dialog",
		ops: "set_path, confirm, submit_path",
		phase: "core",
	},
	{
		name: "automation_shortcut",
		ops: "send, list, describe (app shortcuts)",
		phase: "core",
	},
	{
		name: "automation_task",
		ops: "run, status, cancel, list_profiles",
		phase: "core",
	},
	{
		name: "automation_system",
		ops: "status, info, clipboard, processes, telemetry",
		phase: "2",
	},
	{
		name: "get_window_state",
		ops: "per-window snapshot (ax/som/vision)",
		phase: "core",
	},
	{ name: "get_desktop_state", ops: "full desktop UI tree", phase: "core" },
	{
		name: "automation_mission",
		ops: "run (autonomous), plan, workflow, status, cancel",
		phase: "1,5",
	},
	{
		name: "automation_macro",
		ops: "record, stop, replay, replay_with_verify, list",
		phase: "3",
	},
	{
		name: "automation_smart",
		ops: "discover, list_apps, list_controls, click (by intent)",
		phase: "4",
	},
	{
		name: "automation_watch",
		ops: "start, status, stop, list (event-driven)",
		phase: "5",
	},
	{
		name: "automation_analyze",
		ops: "crawl, discover, portfolio (WinApp UI tree)",
		phase: "core",
	},
	{
		name: "automation_face",
		ops: "opt-in — face recognition (off by default)",
		phase: "opt-in",
	},
	{
		name: "global_keylogger",
		ops: "opt-in — debug hook (off by default)",
		phase: "opt-in",
	},
	{
		name: "approve_automation",
		ops: "grant HITL time window",
		phase: "safety",
	},
	{
		name: "automation_safety",
		ops: "kill switch, rate limits, dry-run status",
		phase: "safety",
	},
];
	...
	return (
		<div className="p-6 max-w-5xl">
			...
			<p className="text-slate-400 text-sm mb-6">
				Local reference. Full docs in the repo under{" "}
			...
			{tab === "start" && (
				<div className="space-y-4 text-sm text-slate-300">
					...
					<div className="bg-slate-950/30 border border-slate-800 rounded-lg p-3">
						<code className="text-xs text-slate-300">
							git clone ... && cd windows-computer-use-mcp && just bootstrap &&
							just serve
						</code>
					</div>
					...
					<ul className="list-disc pl-5 space-y-1">
						<li>
							<code className="text-slate-300">INSTALL.md</code> — detailed
							setup
						</li>
						<li>
							<code className="text-slate-300">docs/SAFETY.md</code> — HITL,
							kill switch, opt-in features
						</li>
						<li>
							<code className="text-slate-300">docs/TOOLS.md</code> — full
							22-tool reference
						</li>
						<li>
							<code className="text-slate-300">docs/py-stack.md</code> — Python
							dependency breakdown
						</li>
						<li>
							<code className="text-slate-300">
								docs/composing-with-playwright.md
							</code>{" "}
			...
			{tab === "tools" && (
				<div className="overflow-x-auto">
							...
							{TOOLS.map((t) => (
								<tr key={t.name} className="border-b border-slate-800/60">
									<td className="py-2 pr-4">
										<code className="text-emerald-400/90 text-xs">
											{t.name}
										</code>
									</td>
									...
									<td className="py-2">
										{t.phase === "core" && (
											<Badge
												variant="outline"
												className="text-[10px] bg-slate-950 text-slate-500 border-slate-700"
											>
												core
											</Badge>
										)}
										{t.phase === "opt-in" && (
											<Badge
												variant="outline"
												className="text-[10px] bg-amber-950/30 text-amber-500 border-amber-800"
											>
												opt-in
											</Badge>
										)}
										...
										{t.phase !== "core" &&
											...
											t.phase !== "safety" && (
												<Badge
													variant="outline"
													className="text-[10px] bg-emerald-950/20 text-emerald-500 border-emerald-800"
												>
													phase {t.phase}
												</Badge>
											)}
					...
					<p className="text-xs text-slate-500 mt-3">
						Phase = when the tool was added. "core" = shipped with the original
						portmanteau release.
					</p>
				</div>
			)}
				...
				<div className="space-y-4 text-sm text-slate-300">
					<div className="bg-slate-950/50 border border-slate-800 rounded-lg p-4">
						<h3 className="text-emerald-400 font-medium mb-2">
							Automation mission (autonomous)
						</h3>
						<p className="mb-2">
							Give it a natural-language goal. The server decomposes it into
							steps via LLM sampling, executes each with retry + verification,
							and returns per-step pass/fail with evidence.
						</p>
						<code className="text-xs text-slate-400 block bg-slate-950 p-2 rounded">
							automation_mission(run="open Notepad, type hello world, save as
							test.txt")
						</code>
					</div>
					<div className="bg-slate-950/50 border border-slate-800 rounded-lg p-4">
						...
						<p className="mb-2">
							Explicit step list with app launch, timeout, and cross-step data
							chaining via $ref: keys.
						</p>
						<code className="text-xs text-slate-400 block bg-slate-950 p-2 rounded">
					...
					<div className="bg-slate-950/50 border border-slate-800 rounded-lg p-4">
						...
						<p className="mb-2">
							Record any UI sequence, save to JSONL, replay with optional
							outcome verification.
						</p>
						<code className="text-xs text-slate-400 block bg-slate-950 p-2 rounded">
							automation_macro(record) → run tools → stop → replay /
							replay_with_verify
						</code>
					</div>
					<div className="bg-slate-950/50 border border-slate-800 rounded-lg p-4">
						<h3 className="text-emerald-400 font-medium mb-2">
							Smart click (intent-based)
						</h3>
						<p className="mb-2">
							Describe what to click in natural language. The tool scans all
							visible windows, finds matching elements, and disambiguates via
							LLM if multiple candidates exist.
						</p>
						<code className="text-xs text-slate-400 block bg-slate-950 p-2 rounded">
					...
					<div className="bg-slate-950/50 border border-slate-800 rounded-lg p-4">
						...
						<p className="mb-2">
							Background thread polls for conditions: window_appears,
							window_closes, text_appears, element_appears.
						</p>
						<code className="text-xs text-slate-400 block bg-slate-950 p-2 rounded">
							automation_watch(start, condition="window_appears",
							target="Update")
						</code>
					</div>
				</div>
				...
				<div className="space-y-4 text-sm text-slate-300">
					<div className="bg-amber-950/20 border border-amber-800/50 rounded-lg p-4">
						<p className="font-medium text-amber-400 mb-1">
							This server drives the real Windows session.
						</p>
						<p className="text-slate-400">
							It is not browser-sandboxed. Read{" "}
							<code className="text-slate-300">docs/SAFETY.md</code> before
							production use.
						</p>
					</div>
					...
					<ul className="list-disc pl-5 space-y-1">
						<li>
							<strong className="text-slate-200">HITL approval</strong> —
							mutating actions require a time window grant via{" "}
							<code className="text-slate-300">approve_automation</code>
						</li>
						<li>
							<strong className="text-slate-200">Kill switch</strong> —{" "}
							<code className="text-slate-300">
								PYWINAUTO_MCP_KILL_SWITCH=1
							</code>{" "}
							blocks all mutations
						</li>
						<li>
							<strong className="text-slate-200">Rate limiting</strong> —
							default 120 mutations/minute, configurable
						</li>
						<li>
							<strong className="text-slate-200">Dry-run mode</strong> — count
							actions without executing them
						</li>
						<li>
							<strong className="text-slate-200">Retry policy</strong> —
							exponential backoff, strategy cascade (refocus → wait_stable →
							fallback → escalate)
						</li>
						<li>
							<strong className="text-slate-200">Self-healing</strong> —
							missions re-launch dead apps, abort after 5 consecutive failures
						</li>
					</ul>
					...
					<table className="w-full text-sm">
						...
						<tbody className="text-slate-300">
							<tr className="border-b border-slate-800/60">
								<td className="py-2 pr-4">
									<code className="text-amber-400/90">automation_face</code>
								</td>
							...
							<tr className="border-b border-slate-800/60">
								<td className="py-2 pr-4">
									<code className="text-amber-400/90">global_keylogger</code>
								</td>
								<td className="py-2 text-xs">
									<code className="text-slate-400">
										PYWINAUTO_MCP_ENABLE_KEYLOGGER=1
									</code>
								</td>
							</tr>
						</tbody>
					...
					<p className="text-xs text-slate-500 mt-2">
						Pair with <code className="text-slate-400">virtualization-mcp</code>{" "}
						for Windows Sandbox / VM isolation.
					</p>
				</div>
			)}
			...
			{tab === "docker" && (
				<div className="space-y-4 text-sm text-slate-300">
					<div className="bg-slate-950/50 border border-slate-800 rounded-lg p-4">
						...
						<p className="mb-2">
							The repo uses its own tools to test its own NSIS installer — 12
							autonomous phases:
						</p>
						<ol className="list-decimal pl-5 space-y-1 text-xs text-slate-400">
							...
							<li>
								Nav click-through (4 pages with sidebar + OCR verification)
							</li>
						...
						<p className="mt-2 text-xs text-slate-500">
							Run with{" "}
							<code className="text-slate-400">just cua-nsis-test</code>. Full
							docs at{" "}
							<code className="text-slate-400">
								docs/cua-nsis-certification.md
							</code>
					...
					<div className="bg-slate-950/50 border border-slate-800 rounded-lg p-4">
						<h3 className="text-emerald-400 font-medium mb-2">
							Tool tests tool — dogfooding map
						</h3>
						<table className="w-full text-sm text-xs">
							...
							<tbody className="text-slate-400">
								<tr className="border-b border-slate-800/60">
									<td className="py-1">
										<code>automation_windows</code>
									</td>
									<td className="py-1">4 (find, maximize)</td>
								</tr>
								<tr className="border-b border-slate-800/60">
									<td className="py-1">
										<code>automation_visual</code> (screenshot)
									</td>
									<td className="py-1">5, 8</td>
								</tr>
								<tr className="border-b border-slate-800/60">
									<td className="py-1">
										<code>automation_visual</code> (OCR)
									</td>
									<td className="py-1">8</td>
								</tr>
								<tr className="border-b border-slate-800/60">
									<td className="py-1">
										<code>automation_elements</code>
									</td>
									<td className="py-1">9 (nav click)</td>
								</tr>
								<tr>
									<td className="py-1">REST API</td>
									<td className="py-1">6, 7</td>
								</tr>
							</tbody>
						</table>
					</div>
